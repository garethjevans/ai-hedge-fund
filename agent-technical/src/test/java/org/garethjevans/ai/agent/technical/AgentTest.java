...
public class AgentTest {
  ...
  public void canInvokeTool() {
    ...
    assertThat(response).containsIgnoringCase("technical");
    response =
        client.prompt("What would the technical analysis agent think about AVGO?").call().content();
  ...
  public SyncMcpToolCallbackProvider getSyncMcpToolCallbackProvider(ChatModel chatModel) {
    List<McpSyncClient> clients =
        List.of(getMcpSyncClient("http://localhost:10096/", ChatClient.create(chatModel)));
