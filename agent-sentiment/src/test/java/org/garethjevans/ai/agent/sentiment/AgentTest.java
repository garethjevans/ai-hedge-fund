...
public class AgentTest {
  ...
  public void canInvokeTool() {
    ...
    assertThat(response).containsIgnoringCase("sentiment");
    response =
        client.prompt("What would the sentiment analysis agent think about AVGO?").call().content();
  ...
  public SyncMcpToolCallbackProvider getSyncMcpToolCallbackProvider(ChatModel chatModel) {
    List<McpSyncClient> clients =
        List.of(getMcpSyncClient("http://localhost:10095/", ChatClient.create(chatModel)));
    List.of(getMcpSyncClient("http://localhost:10095/", ChatClient.create(chatModel)));
